...
Implement the `core.basesyntax.Storage` of the `key-value` type that is parameterized with two types of data. 
...
Example:
```java
core.basesyntax.Storage<Integer, Box> storage = new StorageImpl<>(); 
Box box = new Box();
storage.put(22, box); 
...
If you use arrays for your solution, we assume that the maximum number of elements in our `core.basesyntax.Storage` is 10.
